...
/**
 * Creates a pre-configured error handler for a specific operation.
 * Useful when you have multiple places that might fail in the same way.
 *
 * @param operation - The operation name
 * @param defaultOptions - Default options for this handler
 * @returns A function that handles errors for this operation
 *
 * @example
 * ```typescript
 * const handleHistoryError = createBackgroundErrorHandler("record history", {
 *   showToast: true,
 *   toastMessage: "Could not save to undo history",
 * });
 *
 * // Later, in multiple places:
 * recordOperation(result).catch(handleHistoryError);
 * ```
 */
export function createBackgroundErrorHandler(
  operation: string,
  defaultOptions: Omit<BackgroundErrorOptions, "operation"> = {}
): (error: unknown) => void {
  return (error: unknown) => {
    handleBackgroundError(error, { operation, ...defaultOptions });
  };
}

/**
 * Wraps a promise to handle errors in the background.
 * The promise will never reject - errors are handled internally.
 *
 * @param promise - The promise to wrap
 * @param options - Error handling options
 * @returns A promise that resolves to the result or undefined on error
 *
 * @example
 * ```typescript
 * // Fire and forget with error handling
 * await handleBackgroundPromise(recordOperation(result), {
 *   operation: "record history",
 *   showToast: true,
 * });
 * ```
 */
export async function handleBackgroundPromise<T>(
  promise: Promise<T>,
  options: BackgroundErrorOptions
): Promise<T | undefined> {
  try {
    ...
    handleBackgroundError(error, options);
    return undefined;
  }
}
